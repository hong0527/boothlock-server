...
1. **main에 직접 push 금지** — 모든 변경은 브랜치 → PR로만 올린다
   ...
   - 관리자(홍화수·김재원): 세팅·긴급 수정에 한해 승인 없이 머지(바이패스)할 수 있다. **단 이 경우에도 PR은 반드시 만들고, 본문에 바이패스 사유를 한 줄 남긴다** — 직접 push는 관리자에게도 금지
...
6. **PR 없이 하루 넘게 브랜치를 묵히지 않는다** (= 늦어도 다음 날엔 PR을 올린다. 머지 기한도 하루 — 2번 참조) — 작업 단위를 작게 잡을 것
...
# [6] 리뷰어 지정 — PR 화면 오른쪽 Reviewers에서 5번 표의 같은 그룹원 1명 선택 (또는 gh pr edit --add-reviewer 아이디)
#     승인 + CI(build) 통과 후 → "Squash and merge" 버튼으로 머지
...
- 목표 수명: **만든 다음 날 안에 머지**(기한 하루). 오래 살수록 main과 벌어져 충돌이 커진다 — 실제로 PR이 며칠 밀렸을 때 같은 파일을 건드린 PR끼리 충돌이 났다
...
2. **리뷰어 지정 — 같은 그룹 안에서 아무나 1명**. 기능이 맞물리는 파트끼리 묶어서 서로의 코드를 이해하기 쉽게 했다 (단 0번 3항처럼 남의 파일을 고친 PR은 그룹과 무관하게 **그 파일 담당자**를 지정):

    | 그룹 | 구성원 (파트) | 관리자 |
    | --- | --- | --- |
    | A | 홍화수(주문) · 권희원(메뉴) · 황대겸(계정·부스) | 홍화수 |
    | B | 김재원(대시보드·결제) · 전형준(테이블·세션) · 백지연(정산) | 김재원 |

    - 그룹 안이면 누구에게 요청해도 된다. 3명이라 한 명이 바빠도 나머지 한 명이 받는다
    - **관리자(홍화수·김재원)**: 그룹원이 24시간 안에 리뷰를 못 했을 때 대신 리뷰·승인한다. 자기 그룹이 우선이고, 상대 그룹도 관리자끼리 요청하면 봐준다
    - 관리자 본인의 PR은 같은 그룹의 다른 두 명 중 한 명이 리뷰한다 (자가 승인은 여전히 금지)

3. **PR 본문은 템플릿 4칸을 모두 채운다** (무엇을/왜·명세서 근거/테스트 확인/봐줄 곳). "테스트 확인" 칸에는 **실제로 실행해본 결과**를 적는다 — 안 돌려봤으면 PR을 올리지 않는다
4. **자기 PR 자가 승인 금지** (깃허브가 시스템으로 막는다) — 그룹원의 승인 1개를 받은 뒤, 머지 버튼은 **작성자 본인이** "Squash and merge"로 누른다
5. 머지 후 브랜치 삭제
6. 리뷰어의 의무: 요청받은 지 **24시간 안에** 리뷰 (승인 또는 코멘트). 못 하면 같은 그룹의 다른 사람이나 관리자에게 넘긴다. 코드가 이해 안 되면 "설명해달라"고 요청할 권리가 있고, 작성자는 설명할 의무가 있다
