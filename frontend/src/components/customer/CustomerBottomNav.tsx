...
export default function CustomerBottomNav({ onStaffCall, onOrderHistory, onCart, cartCount = 0 }: CustomerBottomNavProps) {
  return (
    <nav className="fixed inset-x-0 bottom-0 z-40 h-[97px] w-full bg-primary-50" aria-label="손님 메뉴">
      ...
      <button
        type="button"
        onClick={onCart}
        className="absolute left-5/6 top-[21px] flex h-[48px] w-1/6 -translate-x-1/2 flex-col items-center gap-1 text-[14px] leading-[1.2] font-medium tracking-[-0.56px] text-black relative"
      >
