...
- 세션에 저장된 인원수가 있고(`partySize > 0`) **이 세션의 첫 주문**(`existsBySessionId`가 false)이면, 서버가
  `unitPrice=3000`, `qty=partySize`인 `SEAT_FEE` 항목 하나를 자동으로 items에 추가하고 `totalAmount`에 더한다
- 인원수를 저장하지 않았으면(세션에 `partySize`가 없음) 자릿세를 붙이지 않는다 — 조용히 0원으로 빠지는 게
  아니라 애초에 프론트가 저장 실패 시 다음 화면으로 못 넘어가게 막는다(PartySizePage)
- 두 번째 이후 주문에는 붙지 않는다. O14 수기 주문(운영자 직접 입력)에는 개념 자체가 없어 붙지 않는다
- "첫 주문 판정"은 잠금 없이 확인한다 — 같은 세션에서 정말 동시에 서로 다른 첫 주문 두 개가 겹치는 극히
  좁은 창에서만 중복 부과 가능성이 있으나, 파일럿 규모에서 받아들이는 트레이드오프다(멱등키가 "같은 요청
  재시도"만 막고 "다른 카트 두 개"는 안 막는 것과 같은 수준)
