...
v0.5의 9건 중 7건(부스 수 복수·좌석 표시 방식·좌표 입력 주체·홈 화면 담당·파트 분담·O3 합집합·category 값 체계·시딩 형식)은 코드로 확정됐다. 남은 것과 새로 생긴 것만 둔다.
...
- 부스 상세 화면(1.8)은 만들지 않는다. 좌석 3단계(여유/보통/만석) 변환은 클라이언트가 한다
...
- 화면 3단계 권장 임계값: `empty/total ≥ 0.5 → 여유`, `> 0 → 보통`, `= 0 → 만석`
...
```json
{ "unpaidWarning": true, "id": 12, "label": "A-3", "status": "EMPTY", "warning": "미결제 주문 1건 있음" }
```
...
4. `status = EMPTY`
- 주문 데이터는 삭제·변경하지 않는다. 미결제가 있어도 **막지 않는다** — 결제창 '결제 완료'는 O24를 먼저 부르고, '테이블 비우기'는 확인창 뒤 O6만 부른다
...
| **category** | string | — | **`MAIN` / `SIDE` / `DRINK` 대문자 정확 일치(trim 없음), 또는 `null`/생략(분류 없음).** 그 외(`main`, `" MAIN"`, `""`, `ALL`) `400` — message `category: 분류는 MAIN, SIDE, DRINK 중 하나여야 합니다.` |
...
{ "price": 9000 }
{ "category": "DRINK" }   // 분류 변경, null이면 지움, 생략하면 유지
```
...
- 부스 행 `FOR UPDATE` + `refresh`로 최신 `booth.next_table_seq`를 읽는다
- `seq = max(next_table_seq, 기존 "T-N" 라벨 최댓값 + 1)` — O17 저장이 카운터를 되돌려도 스스로 회복
- 정규화 라벨이 이미 있는 번호(예: O2로 만든 `T1`)는 건너뛴다 — 라벨 번호가 카운터와 항상 같지는 않다
- 저장 후 `next_table_seq = seq + 1`을 **컬럼 단독 UPDATE**. 20건 동시 요청에서 라벨 20개 고유(실측)
- 삭제해도 카운터는 줄지 않는다(O26의 하드 삭제 반납 예외 참조)
...
| 이용 이력(세션) **있음** | **soft delete** — `active = false`. 라벨·번호는 반납하지 않음(과거 주문·세션 FK 보존). QR은 C1에서 `404` |
| 이용 이력 **없음** | **완전 삭제**(행 제거, QR 폐기). 라벨이 `T-N`이고 `N == next_table_seq − 1`이면 카운터를 1 반납해 다음 추가가 같은 번호를 다시 낸다. 인쇄한 QR은 무효가 되므로 재출력 필요 |
...
| 부스 상세 1.8 중 소개·메뉴 미리보기·대기 등록 | Later | 좌석 3단계만 E1에 포함 |
