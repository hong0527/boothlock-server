...
| ~~3~~ | ~~인원 선택·자릿세~~ | **v0.6.9에서 확정·구현 완료** — 아래 "인원수 저장·자릿세" 절, 주문 항목 종류(itemType) 참고. 이 행은 이력용으로 남긴다 | — |
...
> 장바구니(2.2)는 클라이언트 로컬 상태 — 서버 API 없음. 인원 선택(자릿세)은 v0.6.9부터 서버에 저장한다(위 "인원수 저장·자릿세" 절). 품절 재검증은 C3에서 수행.
> **손님 여정**: 대표 QR → 홈 화면 `/home`(E2 약도 + E1 자리 현황) → 부스로 이동 → 테이블 QR(C1) → 인원 선택(신규 세션만, 재스캔 복원 시 생략) → 메뉴판(C2) → 주문(C3) → 상태 확인(C4)
>
> **QR은 두 종류다.**
>
> | QR | 붙는 곳 | 찍으면 | 서버 |
...
## 인원수 저장·자릿세 (명세서 밖, 파일럿 전용, v0.6.9)

> 2026-09-24 운영자 시연 피드백 — "인원 선택·자릿세"(§확정 필요 항목 3번, 이전엔 보류)를 뒤집고 실제로 받기로
> 했다. `PartySizePage.tsx`(Figma 289:3672, 인당 3,000원 안내 카드 + 1~20명 스테퍼)는 이미 있었지만 로컬
> 저장만 했다 — 이번에 서버 저장 + 첫 주문 자동 부과로 완성했다. C1 직후 이 화면을 거치도록 손님 플로우를
> 되돌렸다(팀원 PR #62가 "결제 금액에 영향 없다"며 건너뛰게 했던 걸 반전 — 그 전제가 이제 안 맞다). 단,
> **QR 재스캔으로 세션이 복원된 경우(C1 `restored:true`)는 다시 묻지 않고 바로 메뉴판으로 간다.**

### PATCH /api/v1/table-sessions/party-size — 인원수 저장

**Request 헤더**: `X-Session-Token` 필수(누락 401)

**Request Body**: `{ "partySize": 2 }` — 1~20 사이 정수 필수(범위 밖·누락·null·소수·문자열 `400`)

**Response 200**: 본문 없음

**Errors**: `400` / `401` / `410`(세션 만료·퇴실됨 — 저장 직전 조건부 UPDATE로 재확인)

### 자릿세 계산 (C3 주문 생성에 반영)

- 세션에 저장된 인원수가 있고(`partySize > 0`) **이 세션에 청구된 자릿세가 아직 없으면**(취소 안 된 주문에 취소 안 된
  `SEAT_FEE` 항목이 없음), 서버가 `unitPrice=3000`, `qty=partySize`인 `SEAT_FEE` 항목 하나를 items에 추가하고 `totalAmount`에 더한다
- 판정은 주문 저장 트랜잭션 안, **세션 행 잠금 뒤**에 한다 — 같은 테이블 폰 두 대가 동시에 첫 주문을 넣어도 한 번만 붙는다
  (잠금 밖에서 판정하던 초안은 로컬 MySQL 8.0에서 10회 중 10회 이중 부과됐다)
- 자릿세가 붙은 주문이 취소되면(C5·O13, 또는 결제창에서 메뉴를 전부 개별 취소) 청구된 것으로 보지 않아 **다음 주문에 다시 붙는다**.
  메뉴를 전부 개별 취소하면 자릿세도 함께 취소되고 주문은 CANCELED가 된다(자릿세만 남은 접수 주문을 만들지 않는다)
- 취소된 자릿세 주문을 "되돌리기"할 때, 그 사이 다른 주문에 자릿세가 다시 붙었다면 되살린 주문의 자릿세는 빠진다(이중 청구 방지)
- **인원수가 없는 세션의 C3는 `409 PARTY_SIZE_REQUIRED`** (자릿세가 이미 청구된 세션은 예외) — 두 번째 폰·응답 유실 뒤 재스캔은
  C1 `restored:true`라 인원 선택을 건너뛸 수 있었고, 그대로 주문하면 자릿세가 영구히 0원이 됐다. 프론트는 이 코드를 받으면
  인원 선택 화면으로 보냈다가 주문 확인으로 돌아온다. C1 응답의 `partySize`(없으면 null)로 인원 선택 화면을 띄울지 정한다
- O14 수기 주문(운영자 직접 입력)에는 붙지 않고, "청구된 자릿세"로도 세지 않는다
- 자릿세는 C3/C4 응답 `items[]`의 진짜 항목이다(`itemType: "SEAT_FEE"`, `menuId: null`) — 별도 정액 필드가
  아니라 items 배열 안에 있으므로, 주문 취소·환불(O13·O21)과 정산(O19)이 전부 손댈 필요 없이 그대로 맞물린다
- 스태프는 O23/O23b로 자릿세 항목을 수정·취소할 수 없다(항상 `404`) — 결제창(PaymentModal)도 그 줄의
  `-`/`+`/취소 버튼을 비활성화한다
...
```json
{
  ...
  "totalAmount": 27000,
  "items": [
    { "menuId": 3, "menuName": "김치전", "unitPrice": 8000, "qty": 2, "subtotal": 16000, "itemType": "MENU" },
    { "menuId": 5, "menuName": "제로콜라", "unitPrice": 5000, "qty": 1, "subtotal": 5000, "itemType": "MENU" },
    { "menuId": null, "menuName": "자릿세", "unitPrice": 3000, "qty": 2, "subtotal": 6000, "itemType": "SEAT_FEE" }
  ],
  "payment": {
...
**자릿세(명세서 밖, 파일럿 전용, v0.6.9) — items[].itemType**: `"MENU"`(기본) 또는 `"SEAT_FEE"`. 이 세션의 첫 주문이고
["인원수 저장"](#인원수-저장·자릿세-명세서-밖-파일럿-전용-v069) 절로 인원수를 미리 저장해뒀으면, 서버가 `qty`=인원수·`unitPrice`=3,000원인
`SEAT_FEE` 항목을 자동으로 하나 더 붙이고 `totalAmount`에 더한다. `menuId`는 항상 `null`(실제 메뉴가 아니다).
두 번째 주문부터는 붙지 않는다. O14 수기 주문(운영자가 직접 입력)에는 붙지 않는다. `SEAT_FEE` 항목은 O23/O23b로
수정·취소할 수 없다(스태프가 손댈 수 없는 항목 — 시도하면 `404`, "존재하지 않는 항목"과 같은 응답).
주문 전체가 취소·환불되면(O13·O21) 자릿세도 함께 취소·환불된다(주문 단위 처리라 별도 로직 없음).
  ...
  "orders": [
    {
      ...
      "paymentStatus": "UNPAID",
      "totalAmount": 21000,
      "items": [ { "menuId": 3, "menuName": "김치전", "unitPrice": 8000, "qty": 2, "itemType": "MENU" } ],
...
| O19 | GET | `/api/v1/admin/reports/settlement.xlsx` | 7.3 정산 엑셀 (**ADMIN 전용, v0.6.6 — startAt·endAt 구간(최대 31일), createdAt 기준, PAID만**) | Should |
...
## O19. GET /api/v1/admin/reports/settlement.xlsx?startAt=&endAt= — 정산 엑셀 (기능 7.3, **ADMIN 전용**, v0.6.6)

**Request**: `startAt`·`endAt` 둘 다 필수, ISO 8601 datetime(KST, 예 `2026-09-30T22:00:00`. 초는 생략 가능 — 화면의 `datetime-local`은 `2026-09-30T22:00`으로 보낸다). 조회 구간은 `startAt <= createdAt < endAt`(주문 생성 시각 기준). 자정을 넘는 구간(예 `2026-09-30T22:00` ~ `2026-10-01T03:00`)도 그대로 지원한다. **구간 폭은 최대 31일.**

**Response 200**: `application/vnd.openxmlformats-officedocument.spreadsheetml.sheet`, `Content-Disposition: attachment; filename="settlement_{boothId}_{startAt}_{endAt}.xlsx"`(콜론은 파일명에 못 쓰므로 `yyyyMMdd'T'HHmmss` 형식). 시트 2장.

**시트 1 「요약」** — 먼저 열리는 시트. 운영자가 파일을 열자마자 총매출과 잘 팔린 메뉴를 본다.
...
| &nbsp;&nbsp;현금 † | `{현금 환불대기}` | |
| *(빈 줄)* | | |
| 계좌 입금 합계 | `{계좌이체 매출 + 계좌이체 환불대기}` | |
| 현금 보유 합계 † | `{현금 매출 + 현금 환불대기}` | |

† 표시한 행은 **현금 결제가 한 건이라도 있을 때만** 나온다. 현재 프론트는 `confirmOrderPayment`·`confirmTablePayment`가 `BANK_TRANSFER`를 기본값으로 보내고 `CASH`를 보내는 화면이 없어서, 보통은 이 행들이 생략되고 `계좌 입금 합계`가 곧 전체 보유액이다.
| *(빈 줄)* | | |
| 메뉴명 | 판매수량 | 매출액 |
| (메뉴별, 매출액 내림차순) | | |
| 합계 | `{총 수량}` | `{합계}` |

**시트 2 「매출상세」** — 행 = 구간 내 생성된 **결제완료(PAID)** 주문의 취소되지 않은 OrderItem 1건. 컬럼 `주문번호, 테이블, 주문시각, 메뉴명, 수량, 단가, 금액, 결제상태, 결제수단`. 머리글 행은 틀 고정 + 자동 필터, 열 너비는 서버가 지정한다. `결제상태`는 PAID만 뽑으므로 항상 `PAID`다(확인용으로 남겨둔 열).

**규칙**
- **대상 = 결제완료 매출**: v0.6.5까지는 결제 상태 무관 전체 원장이었으나, "운영자가 얼마 벌었는지 한눈에 보는 리포트"로 목적을 좁히면서 `paymentStatus = PAID`만 상세에 넣는다. 결제 상태와 무관한 전체 원장이 필요하면 O10 대시보드를 쓴다
- **환불대기 금액**: `REFUND_NEEDED`(결제됐다가 취소됐지만 환불 송금 전)는 매출이 아니라 상세·총 매출액에서 빠진다. 다만 그 돈은 **아직 운영자 손에 있어서**, 빼고 끝내면 마감 후 거래내역과 대사할 때 설명할 수 없는 차액이 남는다. 그래서 요약에 따로 적는다. `REFUNDED`(환불 송금 완료)는 어느 쪽에도 안 잡힌다
- **결제수단 분리(대사용)**: 매출·환불대기를 모두 `계좌이체`/`현금`으로 쪼개고, `계좌 입금 합계`(= 계좌이체 매출 + 계좌이체 환불대기)와 `현금 보유 합계`(= 현금 매출 + 현금 환불대기)를 따로 낸다. **둘을 합친 숫자 하나만 주면 현금을 받은 부스에서는 그 값이 은행 거래내역과 영영 맞지 않는다** — 계좌이체분은 은행 거래내역과, 현금분은 실제 현금통과 대사해야 하기 때문. O18(`SalesStatsService`)이 `bankTransferSales`/`cashSales`로 나누는 것과 같은 이유다
...
- **구간 상한 31일**: 쿼리에 `Limit`이 없어 구간을 넓게 잡으면 그 부스의 주문·항목이 전부 메모리로 올라오고 워크북도 통째로 조립된다. 파일럿 규모(2~3일 행사)에서 한 달을 넘길 일이 없어 서비스에서 막는다
- **취소 항목 제외**: 개별 취소(O23b)된 항목은 상세·메뉴별 집계·총액 모두에서 뺀다 — C4·대시보드 응답의 "취소 항목 제외" 관례와 통일
- **메뉴별 집계**: 같은 메뉴명끼리 합치고 매출액 내림차순(동률이면 메뉴명 오름차순)으로 정렬. 행사 중 단가를 바꿨어도 수량·금액은 그대로 합산된다
- **권한 = O18과 동일(ADMIN 전용)**: 명세 O19 자체엔 role 제한이 명시돼 있지 않았으나, 같은 "7 정산" 범주인 O18(매출 집계)이 ADMIN 전용이라 통일했다
- 삭제(hidden=true) 처리된 주문도 결제 상태가 PAID면 포함(O18과 같은 이유 — 정산은 숨김 여부와 무관)
- **수식 주입**: xlsx는 셀마다 타입이 박히고 자유 입력 값은 전부 문자열 셀로 쓰므로 엑셀이 수식으로 실행하지 않는다. CSV 시절의 `'` 접두사 처리는 더 필요 없다
- **O18과의 관계**: O18은 영업일(06:00 경계)로 대상을 추린 뒤 PAID를 합산하고, O19는 사용자가 지정한 createdAt 구간의 PAID를 합산한다 — 조회 축이 달라 같은 날짜를 겨냥해도 두 값이 항상 일치하지는 않는다(구간 경계·PAID 전환 시점에 따라 서로 다른 쪽에 잡힐 수 있음)

**Errors**: `400`(`startAt`·`endAt` 누락, `startAt >= endAt`, 구간 31일 초과, 형식 오류) / `401` / `403`(STAFF)
...
| 14 | 정산 파일 수식 주입 방지 | O19 (v0.6.4 CSV `'` 접두사로 구현, v0.6.6 xlsx 전환으로 불필요 — 문자열 셀은 엑셀이 수식으로 실행하지 않음) |
...
| ~~인원 선택·자릿세~~ | **v0.6.9 구현 완료** | `PATCH /table-sessions/party-size` + C3 자동 부과, 이 행 삭제하지 않고 이력용으로 남김 |
